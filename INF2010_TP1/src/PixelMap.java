	...
	protected void AllocateMemory(ImageType type, int h, int w)
	{
		...
		for(int j = 0; j < width; ++j)
		{
			for(int i = 0; i < height; ++i)
			{
				...
				{
					imageData[i][j] = AllocateBWMemory();
				}
				else if(imageType == ImageType.Gray)
				{
					imageData[i][j] = AllocateGrayMemory();
				}
				else if(imageType == ImageType.Color)
				{
					imageData[i][j] = AllocateColorMemory();
				}
				else{
					imageData[i][j] = AllocateTransparentMemory();
				}
			}
		}
	}
	
	private AbstractPixel AllocateBWMemory()
	{
		return new BWPixel();
	}
	
	private AbstractPixel AllocateGrayMemory()
	{
		return new GrayPixel();
	}
	
	private AbstractPixel AllocateColorMemory()
	{
		return new ColorPixel();
	}
	
	private AbstractPixel AllocateTransparentMemory()
	{
